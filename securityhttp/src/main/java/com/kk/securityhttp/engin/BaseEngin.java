...
public abstract class BaseEngin<T> {
    ...
    //< 同步请求rxjava post 1
    public Observable<T> rxpost(final Type type, final Map<String, String> params, final Map<String, String> headers, final boolean isrsa, final boolean iszip, final boolean isEncryptResponse) {
        return Observable.just("").map(new Func1<Object, T>() {
            @Override
            public T call(Object o) {
                LogUtils.i("同步请求rxjava->" );
                return post(type, params, headers, isrsa, iszip, isEncryptResponse);
            }
